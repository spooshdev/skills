...
// Invalidate modes
await trigger({ body: data, invalidate: "all" });       // Full path hierarchy
await trigger({ body: data, invalidate: "self" });      // Exact path only
await trigger({ body: data, invalidate: "none" });      // No invalidation
await trigger({ body: data, invalidate: "*" });         // Global refetch all queries
await trigger({ body: data, invalidate: ["posts"] });   // Specific tags
await trigger({ body: data, invalidate: ["all", "dashboard"] });  // Mode + extra tags
```
...
invalidate("users");           // Single tag
invalidate(["users", "posts"]); // Multiple tags
